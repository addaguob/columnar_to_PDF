...
def main():
    ...
    display_patients(input_patient_info())
    display_columnar()
    wait_for_event()
...
def input_patient_info():
    ...
    def ui_entry(master, txt_entry):
        ...
        txt_entry[WIDGET] = ui.Entry(
            master=master, fg=PLACEHOLDER_COLOR, font=FONT, width=40
        )
        ...
        txt_entry[WIDGET].pack(
            ...
            pady=PADY,
            ipadx=IPADX,
            ipady=IPADY + 20 if "Rx" in txt_entry[PLACEHOLDER_VALUE] else IPADY,
        )
    ...
    contact_no = ui_entry(frame_px_info_left, [None, "", "Contact Number"])
    ...
    total_amount = ui_entry(frame_px_info_left, [None, "", "Total Amount"])
    ...
    ui.Button(
        master=frame_proceed, text="PROCEED", font=FONTBIG, command=px_UI.destroy
...
def display_patients(*columns):
            ...
            with open(file_path, "r", newline="") as csvfile:
                ...
                for column in header:
                    ...
                    tree.column(
                        ...
                        width=(
                            20
                            if column in ("age", "lens")
                            else 100 if column not in ("address", "occupation") else 200
                        ),
                    )
                for row in csv_reader:
                    tree.insert("", "end", values=row)
                label_status.config(text=f"Patients CSV File loaded: {file_path}")
    ...
    ui.Button(
        master=px_columnarUI,
        text="EXIT COLUMNAR",
        font=FONTBIG,
        command=px_columnarUI.destroy,
    ).pack(pady=5)
